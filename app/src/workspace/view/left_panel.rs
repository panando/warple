...
impl LeftPanelView {
    pub fn new(
        ...
        let active_view = ToolPanelView::ProjectExplorer; // 只使用 ProjectExplorer
        // 只保留 ProjectExplorer，过滤掉其他所有视图
        let filtered_views: Vec<ToolPanelView> = views.iter().filter(|v| matches!(v, ToolPanelView::ProjectExplorer)).copied().collect();
        let toolbelt_buttons = filtered_views
            .iter()
    ...
    pub fn update_available_views(
        ...
        // 只保留 ProjectExplorer，过滤掉其他所有视图
        let filtered_views: Vec<ToolPanelView> = views.iter().filter(|v| matches!(v, ToolPanelView::ProjectExplorer)).copied().collect();
        self.toolbelt_buttons = filtered_views.iter().map(|view| Self::create_toolbelt_button_config(view, ctx)).collect();
